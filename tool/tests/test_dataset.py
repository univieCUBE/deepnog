...
class TestDataset:
    ...
    def test_multiprocessDataLoading(self, num_workers, f_format='fasta'):
        ...
        s = set()
        for i, el in enumerate(DataLoader(dataset, num_workers=0, 
                                          batch_size = 1, 
                                          collate_fn=collate_sequences)):
                # Check uniqueness of IDs for loaded data
                for identifier in el.ids:
                    assert((identifier in s) == False)
                    s.add(identifier)
    ...
    def test_correctCollatingSequences(self, batch_size, f_format='fasta'):
        ...
        test_file = "./tests/data/GCF_000007025.1.faa"
        dataset = ProteinDataset(test_file, f_format=f_format)
        for i, el in enumerate(DataLoader(dataset, 
                                        batch_size=batch_size, 
                                        num_workers=4, 
                                        collate_fn=collate_sequences)):
            assert(el.sequences.shape[0] == batch_size)
            assert(len(el.ids) == batch_size)
            break
    ...
    def test_zeroPadding(self, f_format='fasta'):
        ...
        for i, el in enumerate(DataLoader(dataset, 
                                        ...
                                        collate_fn=collate_sequences)):
            # Test correct shape
            assert(el.sequences.shape[1] == 112)
            # Test correctly zeros inserted
            assert(sum(el.sequences[0,56:]) == 0)
    ...
    def test_correctEncoding(self, f_format='fasta'):
        ...
        for i, el in enumerate(test_encoded):
            assert((i+1) == el)